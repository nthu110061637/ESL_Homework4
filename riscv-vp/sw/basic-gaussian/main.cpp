...
// Gassian Filter ACC
static char* const GassianFILTER_START_ADDR = reinterpret_cast<char* const>(0x73000000);
static char* const GassianFILTER_READ_ADDR  = reinterpret_cast<char* const>(0x73000004);
    ...
    for(int i = 0; i < height; i++){
      ...
      for(int v = -1; v <= 1; v ++){
        for(int u = -1; u <= 1; u++){
          ...
          write_data_to_ACC(GassianFILTER_START_ADDR, buffer, 4);
        }
      }
      read_data_from_ACC(GassianFILTER_READ_ADDR, buffer, 4);
