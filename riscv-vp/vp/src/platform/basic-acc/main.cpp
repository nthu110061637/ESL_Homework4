...
public:
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > gaussianFilter_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	bus.ports[12] = new PortMapping(opt.gaussianFilter_start_addr, opt.gaussianFilter_end_addr);
	...
	bus.isocks[12].bind(gaussian_filter.tsock);
